...
function make_certificate_line($text1,$text2,$text3,$image, $color,$y) {
    ...
    // Calculate widths
    $box1 = imagettfbbox($size_ovo, 0, $font_ovo, $text1);
    $w1 = $box1[2] - $box1[0];

    $box2 = imagettfbbox($size_chalkboard, 0, $font_chalkboard, $text2);
    $w2 = $box2[2] - $box2[0];

    $box3 = imagettfbbox($size_ovo, 0, $font_ovo, $text3);
    $w3 = $box3[2] - $box3[0];
    ...
    // Draw text1
    imagettftext($image, $size_ovo, 0, $x_start, $y, $color, $font_ovo, $text1);

    // Draw text2
    imagettftext($image, $size_chalkboard, 0, $x_start + $w1, $y, $color, $font_chalkboard, $text2);

    // Draw text3
    imagettftext($image, $size_ovo, 0, $x_start + $w1 + $w2, $y, $color, $font_ovo, $text3);
}
...
function add_wrapped_text_to_image($image, $text, $font_path_regular, $font_path_bold, $font_size, $color, $highlight_color, $highlight_words, $image_width, $alignment, $y) {
    ...
    foreach ($words as $word) {
        ...
        if ($line_width > $image_width - 20) { // 20px padding from both sides
            draw_text_line($image, $line, $font_path_regular, $font_path_bold, $font_size, $color, $highlight_color, $highlight_words, $image_width, $alignment, $current_y);
            $line = $word;
...
function draw_text_line($image, $line, $font_path_regular, $font_path_bold, $font_size, $color, $highlight_color, $highlight_words, $image_width, $alignment, $y) {
    ...
    foreach ($words as $word) {
        ...
        $x += $word_width + 5; // 5px space between words
    }
}


function make_text_middle($name,$image,$color,$font_path,$add_x_value=0,$add_y_value=0,$font_size=150){
    // Get image dimensions
    $image_width = imagesx($image);
    $image_height = imagesy($image);

    $text_bound = imageftbbox($font_size, 0, $font_path, $name);
    ...
    // Add text to image
    return imagettftext($image, $font_size, 0, $start_x_offset+$add_x_value, $start_y_offset+$add_y_value, $color, $font_path, $name);

    list($left,, $right) = imageftbbox(18, 0,  $font_path, $name);
    $twidth = $right - $left;
    $pos = ($image_width = imagesx($image) - ($twidth-55)) / 2;
